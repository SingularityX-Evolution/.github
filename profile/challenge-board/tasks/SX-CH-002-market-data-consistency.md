...
| 当前状态 | 已关闭（S、A 均关闭） |
| 关闭日期 | 2026-08-03 |
| 认定关闭规则 | 挑战已整体关闭；不再接受新的 S / A 认定申请 |
| 提交方式 | 已停止接收新提交 |
| 提交机会 | 已停止 |

> 本页面为公开展示版，展示背景、核心问题、公开任务范围、交付标准、认定标准以及赏金分档。赏金为上限，不是保证付款；实际支付以隐藏评测、复现结果、评审结论、合规要求和最终接收规则为准。

> **挑战关闭公告（2026-08-03）：** 本挑战已整体关闭，S、A 两档均不再接收新的提交或认定申请。关闭公告前已经完整送达的材料，按原有队列与回复规则处理。下文保留历史题面与认定规则，仅供查阅。
...
理论上，这两类数据描述的是同一交易所、同一合约、同一市场事件。但实际使用中发现：即使尽量对齐数据字段、时间窗口和计算逻辑，Tardis 历史数据与实时采集数据仍然存在差异，并且这种差异会传导到 10 秒级因子计算结果中。我们可以提供的Tardis数据源https://data.yutsing.work/0416-qml/tardis/binance-futures/ ，该数据源每天10:00（UTC+8）会更新前一天(UTC+0)的数据，
...
**当前状态：本挑战已于 2026-08-03 整体关闭。以下内容为关闭前适用的历史认定规则。**

本题采用“S / A 等级认证、单档关闭、按完成度分档支付”机制：

1. 每位提交者对本挑战仅有 1 次首次提交机会。仅当首次提交获得 A 档认证时，可自 A 档认证确认邮件发出之时起 5 个自然日（120 小时）内，再提交 1 个完整升级版本；期限以主办方邮件系统记录的发送时间为准，逾期视为放弃升级机会。每人对本挑战合计最多 2 次提交。
2. 升级提交仅用于申请 S 档认证，并须作为新的完整、独立且不可修改的版本单独计次。达到 S 档标准的，最终按 S 档认证和结算；如 A 档奖金已经支付，仅补发 S 档奖金与已支付 A 档奖金之间的差额，不重复领取两档奖金。未达到 S 档的，原 A 档认证继续有效，不产生新的 A 档认定或奖金。
...
5. 每次提交均以完整材料送达主办方指定邮箱或指定上传渠道的时间戳为准进入评审队列；A→S 升级机会不重新开放已经关闭的 S 档，仅可在升级提交送达时 S 档仍开放的情况下使用。关闭公告前已经收到的提交，主办方仍会给出答复，但不保证该档位仍有奖金名额或继续进入该档奖金评审。
6. 认证成功者不自动获得全额，实际支付金额将根据隐藏评测集中的数据一致性、因子一致性、工程可运行性、差异归因完整度和报告完整度分档确定。
